...
def infer(engine, image):
    ...
    for binding in engine:
        ...
        # Associar o endereço do tensor ao binding
        context.set_tensor_address(binding, int(device_mem))
    
    # Pré-processar e copiar imagem para a GPU
    input_image = preprocess_image(image, INPUT_SIZE)
    np.copyto(inputs[0]["host"], input_image.ravel())
    cuda.memcpy_htod_async(inputs[0]["device"], inputs[0]["host"], stream)
    
    # Executar inferência assíncrona
    context.execute_async_v3(stream_handle=stream.handle)
    
    # Copiar saída da GPU para a CPU
    cuda.memcpy_dtoh_async(outputs[0]["host"], outputs[0]["device"], stream)
    stream.synchronize()
    
    # Reshape da saída
    output = outputs[0]["host"].reshape(1, -1, 5+len(CLASSES))
    return output
...
def main():
    ...
    # Configurar captura de vídeo usando a webcam padrão
    cap = cv2.VideoCapture(0)  # 0 é o índice padrão da primeira câmera
